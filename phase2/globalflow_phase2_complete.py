...
EMERGENCY_PREMIUM = 2.5        # multiplier on cheapest baseline arc for emergency arcs (F infeasible)
...
for SCENARIO_KEY in ALL_SCENARIO_KEYS:
    for STRATEGY in STRATEGIES:
        ...
        if STRATEGY == 'F' and is_infeasible:
            print(f"  [F] Injecting emergency arcs (×{EMERGENCY_PREMIUM} premium)...")
            for s, prods in stranded.items():
                s_full_arcs      = set(arcs_df_all[arcs_df_all['from_id'] == s]['arc_id'])
                baseline_costs_s = [v for (a, p), v in baseline_var_cost.items()
                                    if a in s_full_arcs and p in prods]
                ...
                print(f"    {s} ({sorted(prods)}): "
                      f"base={base_cost:.2f}  emg={emg_unit_cost:.2f}  "
                      f"→ {len(H)} arcs added")
